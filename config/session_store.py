...
class SessionStore:
    ...
    Sessions are persisted as JSON files in ~/.config/Mira/sessions/
    Each session contains metadata and the full message history.
    """
    
    SESSIONS_DIR = Path.home() / ".config" / "Mira" / "sessions"
    CURRENT_SESSION_FILE = Path.home() / ".config" / "Mira" / "current_session.txt"
